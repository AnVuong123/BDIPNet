...
class RBFExpansion(nn.Module):
    ...
    def __init__(
        ...
        if lengthscale is None:
            ...
            d_max = 8
            alpha=1
            if self.edge_type == "coulomb":
                # x = -1/d
                x_min = -alpha / d_min
                x_max = -alpha / d_max

            elif self.edge_type == "vdw":
                # x = -1/d^6
                x_min = -alpha / (d_min**6)
                x_max = -alpha / (d_max**6)
            ...
            "centers", torch.linspace(x_max, x_min, bins)
                 )


            self.lengthscale = np.abs(np.diff(self.centers)).mean()
